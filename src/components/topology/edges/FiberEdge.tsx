...
import { Cable, ChevronDown, ChevronUp, Check, Settings2, Zap, Edit3 } from "lucide-react";
  ...
  return (
    <>
      ...
      <EdgeLabelRenderer>
        ...
        >
          {/* Collapsed view - PROMINENT cable + splice summary */}
          {!isExpanded ? (
            <div
              className={`
                flex items-center rounded-lg
                bg-white/98 backdrop-blur-sm border-2 shadow-lg
                hover:shadow-xl hover:scale-105 transition-all duration-200
                ${selected ? "ring-2 ring-blue-500 border-blue-300" : "border-gray-200"}
              `}
            >
              {/* FIBER COUNT - Large and prominent */}
              <button
                onClick={(e) => {
                  e.stopPropagation();
                ...
                className={`
                  flex items-center gap-2 px-3 py-1.5 transition-colors group rounded-l-lg
                  ${data?.isNew || data?.cable?.name === "New Cable"
                    ? "bg-amber-100 hover:bg-amber-200 animate-pulse"
                    : "hover:bg-amber-50"}
                `}
                title="Click to change fiber count and cable properties"
              >
                ...
                <Settings2 className="w-3.5 h-3.5 text-gray-300 group-hover:text-amber-500 transition-colors" />
              </button>

              {/* SPLICES - Prominent edit button with status */}
              <button
                onClick={(e) => {
                  e.stopPropagation();
                  onOpenEdgeEditor?.(id);
                }}
                className="flex items-center gap-1.5 px-3 py-1.5 hover:bg-blue-50 transition-colors border-l-2 border-gray-200 group"
                title="Edit fiber splices"
              >
                <Zap className="w-4 h-4 text-blue-500 group-hover:text-blue-600" />
                <span className="text-xs font-semibold text-blue-600 group-hover:text-blue-700">Splices</span>
                {hasFiberData ? (
                  <div className="flex items-center gap-1 bg-green-50 px-1.5 py-0.5 rounded-full">
                    <Check className="w-3 h-3 text-green-600" />
                    <span className="text-xs font-bold text-green-700">{connectionStats.completed}</span>
                  </div>
                ) : (
                  <span className="text-[10px] text-gray-400 bg-gray-100 px-1.5 py-0.5 rounded-full">0</span>
                )}
              </button>
              ...
              <button
                ...
                className="flex items-center px-2 py-1.5 hover:bg-gray-50 transition-colors border-l border-gray-200 rounded-r-lg"
                title="Show fiber details"
              >
